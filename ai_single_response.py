...
parser = argparse.ArgumentParser(description="submit a message and have the 335M parameter peter szemraj model respond")
parser.add_argument("--prompt", required=True,
                    help="the message the bot is supposed to respond to")
...
parser.add_argument("--responder", required=False,
...
if __name__ == "__main__":
    ...
    p_list = []
    st = time.time()
    if speaker is not None: p_list.append(speaker + ":" + "\n")  # write prompt as the speaker
    ...
    try:
        ...
        this_result = [str(msg) for msg in diff_list if (":" not in str(msg)) and ("szemr" not in str(msg)) and (
                "peter" not in str(msg))]  # remove all names
    ...
    if want_rt: print("took {runtime} seconds to generate. \n".format(runtime=rt))
